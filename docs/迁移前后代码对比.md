...
├── state.py       # 状态TypedDict    ├── tools.py          # 工具+tool_map+invoke_tool
├── nodes.py       # 所有节点函数     ├── components/       # 组件目录
├── router.py      # 路由函数         │   ├── __init__.py
├── graph.py       # 图构建           │   ├── think_comp.py
├── main.py        # 主入口           │   ├── select_tool_comp.py
├── tools/         # 工具目录         │   └── judge_comp.py
│   ├── __init__.py                   ├── routers.py        # 路由函数
│   ├── calculator.py                 ├── workflow.py       # 工作流构建
│   └── weather.py                    └── main.py           # 主入口
```

**差异**：
- LangGraph 使用函数作为节点，openJiuwen 使用组件类
- LangGraph 需要显式定义 `TypedDict` 状态类，openJiuwen 通过 `inputs_schema` 传递
- openJiuwen 将节点拆分为独立组件文件，便于维护和测试
